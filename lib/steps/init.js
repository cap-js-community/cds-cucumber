...
function infoProcess(process) {
  if(process.pid===0)
    console.log(`Connected to process '${process.params.stack}'`);
  else
...
* @name Given we have started the CAP application
* @description Starts the cds service.
* @example Given we have started the CAP application
*/
...
*/
Given(/we have started the (CAP )?application/, async function(_) {
...
* @param {string} directory directory where the application is located
* @description Starts the cds service of the specified application.
    The application name is passed as a parameter to the cds command.
...
*/
Given('we login with user {string} via {string}', async function( user, endpoint ) {
  await this.controller.authenticate(endpoint, user);
})
