...
When('we open value help for field {string}', async function( field ) {
  if(await this.controller.getVisiblePopoverCount() > 0)
    throw Error('There is already a visible popover!');
...
When('field {string} has value help', async function( fieldName ) {
  if(!await this.controller.hasValueHelp(fieldName))
    throw Error(`${fieldName} has no ValueHelp`);
})

// internal
When('field {string} has no value help', async function( fieldName ) {
  if(await this.controller.hasValueHelp(fieldName))
    throw Error(`${fieldName} has ValueHelp`);
})
