...
Given('we have started embedded server', async function() {
  if(getRemotePort()) { // remote service port configured - do not start embedded server
...
/**
* @name Given we require communication protocol {string}
* @description require the specified protocol to use for communication with the service
* @param {string} protocol name to use for communication
* @example Given we require communication protocol "odata-v2"
*/
Given('we require communication protocol {string}', async function( communicationProtocol ) {
...
Given('we have connected to service {word}', async function( serviceName ) {
  await configureRemoteService(this, {serviceName});
...
*/
Given('we have connected to service {word} as user {string} with password {string}', async function( serviceName, username, password ) {
  if(this.process) {
    const loadModel = require('../common/loadModel');
    await loadModel();
  }
  let serviceNameWithUser = await configureRemoteService(this, {serviceName, username, password});
...
*/
Given('we have connected to service {word} in directory {string} as user {string} with password {string}', async function( serviceName, directory, username, password ) {
  let serviceNameWithUser = await configureRemoteService(this, {serviceName, username, password, directory});
  await connect(this, serviceNameWithUser);
})

async function connect(that, serviceName) {
  if(cds.failedServices && cds.failedServices[serviceName]) // prevent to connect twice to failing services as it blocks the execution
  ...
  try {
    ...
    if(getRemotePort(this))
      await that.cdsService.get('/'); // perform single GET to check the connection
...
async function configureRemoteService(that, args) {
  let { serviceName, port, username, password, directory, path, protocol } = args;
  port ??= getRemotePort(that);
  protocol ??= getCommunicationProtocol(that);
  path ??= that.process?.endpoints?.filter(s=>s.service==serviceName && s.kind==protocol)[0]?.path;
  if(!path) {
    let prefix = {'odata-v2':'/odata/v2/', 'odata-v4':'/odata/v4/'};
    if(!prefix[protocol]) throw Error(`Protocol '${protocol}' no supported, available: ${Object.keys(prefix)}`);
    path = prefix[protocol]+((serviceName.endsWith('Service')?serviceName.substring(0,serviceName.length-7):serviceName)).toLowerCase();
  }
  if(!path) throw Error(`Path for service ${serviceName} with protocol ${protocol} not found`);
  ...
  if(!path) path = cds.model.definitions[serviceName].path || cds.model.definitions[serviceName]["@path"];
  if(!path) path = '/odata/v4/';
  if(protocol=='odata-v2')
    path = path.replace('/odata/v4/','/odata/v2/');
  else if(protocol!='odata-v4')
    throw Error(`Communication protocol '${protocol}' not supported`);
...
function getCommunicationProtocol(that) {
  return env.CDS_CUCUMBER_COMMUNICATION_PROTOCOL || that?.communicationProtocol || 'odata-v4';
}

function getRemotePort(that) {
  return env.REMOTE_PORT || that?.process?.port || that?.port;
}
...
function getEntity(entity) {
  ...
  if(!result)
    result = cds.model.entities[`${this.serviceName}.${entity}`]
  return result;
}
...
function getResultKeys(result=undefined) {
  ...
  if(Array.isArray(instance))
    return instance.map(this.getResultKeys)
  ...
  for(let key in resultKeys) {
    ...
    else if(typeof resultKeys[key] === 'string')
      resultKeys[key] = `'${resultKeys[key]}'`; // quote strings
  }
  return resultKeys;
}
...
async function selectFrom(instance=undefined) {
  ...
  this.prepare.expand.forEach(item => {
    if(typeof item === 'string')
      columns.push({ref:[item], expand:['*']})
    else
      columns.push({ref:[item.field], expand:[{ref:item.select}]})
  })
  ...
  if(this.prepare.limit !== undefined) {
    if(this.prepare.offset !== undefined)
      query.limit(this.prepare.limit, this.prepare.offset);
    else
      query.limit(this.prepare.limit);
  }
