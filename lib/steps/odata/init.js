...
* @description Initializes the CAP service in embedded mode utilizing the model if the current application, 
...
Given('we have connected to service {word}', async function( serviceName ) {
  ...
  if(port)
    await configureRemoteService(serviceName, port);
...
async function configureRemoteService(serviceName, port, username, password, directory) {
  ...
  let path = cds.model.definitions[serviceName].path
