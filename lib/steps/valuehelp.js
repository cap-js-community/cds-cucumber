...
When('we open value help for filter field {string}', async function( field ) {
  ...
  await this.controller.waitEvents();
  let hasPopup = await this.controller.hasVisiblePopoverForFilterBarField(field);
  if(!hasPopup) {
    throw Error(`Missing visible popup for field '${field}'`);
  }
})
...
* @param {string} columnName name of the target column
* @param {string} searchText value to search for in the target column
* @example When we select additional row in value help dialog having field "Travel Status" equal to "Open"
*/
