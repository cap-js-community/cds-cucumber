class CDS {
  ...
  getVisiblePage() {
    return this.getComponents("sap.m.Page").filter(X => x.isVisibleContainer(X))[0];
  }
  ...
  findTableByTitle(title) {
    let pageName = this.getCurrentPage().getParent().getId();
    let re = new RegExp(`${pageName}--fe::table::.*::LineItem-title$`)
    let re1 = new RegExp(`${pageName}--fe::FacetSection::.*::LineItem$`)
    let a = this.getComponentsByIdRe([re,re1])
    let titles = this.getComponentsByIdRe([re,re1])
      .filter(X=>X.getText&&X.getText()||X.getTitle&&X.getTitle())
      .map(X => {
        if(X.getText) {
  ...
  findActiveDialog() {
    return this.getComponents('sap.m.Dialog').filter(X => X.isActive())[0];
  }
  ...
  pressButtonInActiveDialog( label ) {
    ...
    if(!button) throw Error(`pressButtonInActiveDialog(${botton}): button not found`);
    return !!button.firePress();
  }
  ...
  findShellSearchByIcon() {
    return this.getComponents()
      .filter(X => X.getMetadata)
      .filter(X => X.getMetadata().getName() === 'sap.ushell.ui.shell.ShellHeadItem')
  ...
  findHeaderField(fieldLabel) {
    ...
    let forms=this.getComponents().filter(X => X.getId()==formName);
  ...
  modifyField(fieldLabel, newText) {
    ...
    if(field.getMetadata().getName()=='sap.ui.mdc.FilterField') {
      let vlh = field.getFocusElementForValueHelp();
      $(this.jqid(vlh.getId())).click();
      return false;
    }
  ...
  hasVisiblePopoverForFilterBarField(fieldName) {
    let dialog = this.getComponents("sap.m.Dialog").filter(X=>X.isOpen())[0];
    if(dialog) {
      let valueHelpFieldId = dialog.getParent().getParent().getId();
      let replaced = valueHelpFieldId.replace('::FilterFieldValueHelp::','::FilterField::');
      let fieldId = this.findFilterBarField(fieldName).getId();
      if(replaced!=fieldId)
        throw Error(`hasVisiblePopoverForField(${fieldName}): focus IDs differ - ${replaced}!=${fieldId}`);
      return true;
    }
    let pops = this.getComponents("sap.m.Popover");
    let opened = pops.filter(X=>X.oPopup.isOpen());
    let found = opened[0];
    if(!found) return false;
    let popupInitialFocus = found.getInitialFocus()
    let openByParentId = found._oOpenBy.getParent().getId();

    let field = this.findFilterBarField(fieldName);
    if(!field)
      throw Error(`hasVisiblePopoverForField(${fieldName}): field not found`);
    if(field.getId()!=openByParentId)
      return false;
    let focusElementForValueHelpId = field.getFocusElementForValueHelp().getParent().getId();
    if(popupInitialFocus!=focusElementForValueHelpId)
      throw Error(`hasVisiblePopoverForField(${fieldName}): focus IDs differ - ${popupInitialFocus}!=${focusElementForValueHelpId}`);
    return true;
  }
  ...
  findFieldValueHelp(name) {
    ...
    let found = this.getComponentsById(field.getFieldHelp())
    if(found.length!=1) throw Error(`findFieldValueHelp(${name}): Failed to find component, length: ${found.length}`);
    return found[0];
  }

  selectItemInValueHelp(fieldName, text) {
    if(this.hasOpenedValueHelpPopover())
      return this.selectItemInValueHelpPopup(fieldName, text);
    return this.selectItemInFieldValueHelp(fieldName, text);
  }

  selectItemInValueHelpPopup(fieldName,text) {
    let field = this.findField(fieldName);
    let fieldFieldValueHelpId = field.getFields()[0].getContent().getContentEdit()[0].getFieldHelp();
    ...
    if(fieldFieldValueHelpId!=popoverFieldValueHelpId)
      throw Error(`selectItemInValueHelpPopup() - id mismatch ${fieldFieldValueHelpId}!=${popFieldValueHelpId}`);
    let table = popover.getContent()[0].getTable(); //sap.m.Table
    this._selectItemInTable(table, text)
  }
  ...
  _selectItemInList(list, text) { // sap.m.List
    ...
    if(found.length!=1) {
      let available = items.map(X=>X.getLabel())
      throw Error(`_selectItem: Failed to find one matching item, length: ${found.length}, available: ${available.join(', ')}`);
    }
    let listItem = found[0];
    list.fireItemPress({listItem});
  }

  _selectItemInTable(table, value) {
  ...
  selectFirstMatchingRowInValueHelpDialog(columnName, searchText) {
    let innerTable = this.getInnerTableInValueHelpDialog();
    let plugin = innerTable._getSelectionPlugin();
  ...
  selectAdditionalRowInValueHelpDialog(columnName, searchText) {
    let innerTable = this.getInnerTableInValueHelpDialog();
    let plugin = innerTable._getSelectionPlugin();
    let rowIndex = this.findRowIndexInValueHelpDialog(columnName, searchText);
    plugin.addSelectionInterval(rowIndex,rowIndex);
    return rowIndex;
  }
  ...
  openValueHelpDialogForField(fieldName) {
    if(this.findActiveDialog())
      this.openValueHelpDialogForDialogFilterField(fieldName);
    else if(this.getVisibleObjectPage())
      this.openValueHelpDialogForObjectField(fieldName);
    else
      this.openValueHelpDialogForFilterField(fieldName);
  }

  openValueHelpDialogForObjectField(fieldName) {
    let field = this.findField(fieldName);
    let edit = field.getFields()[0].getContent().getContentEdit()[0];
    //$(this.jqid(edit.getId())).find('input')[0].focus();
    edit.onfocusin(); // shortcut
