class CDS {
  ...
  pressGoBackButton() {
    let buttons = this.getComponents('sap.ushell.ui.shell.ShellHeadItem')
        ...
        .filter(X => X.getProperty('ariaLabel')==='Go Back');
  ...
  getObjectPageName() {
    let ops = this.getObjectPages();
    if(ops.length!=1) throw Error('No Object Page located, length: '+ops.length)
    return ops[0].getParent().getId();
  }
  ...
  dumpFormContainer(formContainer, data) {
    ...
    formElements.forEach(E => {
      ...
      let value = this.reduce(fields
        ...
        .map(F => F.getContent().getContentDisplay().getText())
      );
  ...
  findSearchFieldInFilterBar() {
    ...
    //sap.m.SearchField
    let searchField = basicSearchField._getContent()[0];
    return searchField;
  }
  ...
  getVisibleObjectPage() {
    return this.getObjectPages()
      .filter(X => this.hasVisibleContainer(X))[0];
  }
  ...
  pressOkInValueHelpDialog() {
    ...
    let okButton = dialog._getContainer().getButtons().filter(B => B.getText()=="OK")[0];
