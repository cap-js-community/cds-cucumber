...
class Controller {
  ...
  async loadLibrary() {
    this.libUrl = env.CDS_BDD_LIB_URL || '/x.js';
    let src = `
    ...
    if(env.WAIT_DEBUGGER=="1")
      await this.waitDebuggerToAttach()
    return rc;
  }

  async waitDebuggerToAttach(seconds=60) {
    process.stdout.write(`\nImported library: ${this.libUrl}\n`);
    process.stdout.write(`Attach debugger to port ${this.remoteDebuggingPort} in ${seconds} seconds and execute:\nwindow.debuggerAttached=true\n`);
    for(let loops=seconds; loops>0; loops--) {
      let rc = await this.driver.executeScript('return window.debuggerAttached;');
      if(rc) {
        process.stdout.write('!');
        return;
      }
  ...
  async selectItemInValueHelp(field, text) {
    return await this.call("selectItemInValueHelp", field, text);
  }
  ...
  async selectAdditionalRowInValueHelpDialog( columnName, searchText ) {
    return await this.call("selectAdditionalRowInValueHelpDialog", columnName, searchText);
  }
  ...
  async takeScreenshot() {
    let filename = "/tmp/cap.screenshot."+(new Date().toISOString())+".png";
  ...
  async _call(name,...args) {
    ...
    if(env.SLOWDOWN && name!="getEvents")
      await this._sleep(1000*parseInt(env.SLOWDOWN));
  ...
  async waitToLoad( maxLoops = 20 ) {
    ...
    for(let loops=maxLoops; loops>0; loops--) {
      ...
      if(error) {
        let messages = await this.extractFooterBarMessages();
