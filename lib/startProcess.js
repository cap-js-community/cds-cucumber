...
const npx = path.sep == '/' ? 'npx':'npx.cmd';
...
async function startProcess(params={}) {
  ...
  if(env.REMOTE_PORT) {
    params=Object.assign(params,{workingDirectory: createWorkingDirectory()})
    return {pid:0,params};
  }
  let rootdir = cwd();
  ...
  params.workingDirectory = createWorkingDirectory();
...
function createWorkingDirectory() {
  ...
  let tmpdir = (path.sep == '/')
    ?
      path.join(os.tmpdir(), `cds_cucumber_${os.userInfo().username}`, `${process.pid}_${date}`)
    :
      path.join(os.tmpdir(), `cds_cucumber_${os.userInfo().username}_${process.pid}_${date}`)
    ;
